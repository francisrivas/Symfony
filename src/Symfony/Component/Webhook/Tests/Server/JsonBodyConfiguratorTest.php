...
use Symfony\Component\Webhook\Server\PayloadEncoderInterface;

class JsonBodyConfiguratorTest extends TestCase
{
    public function testPayloadWithPayloadEncoder()
    {
        $payload = ['foo' => 'bar'];

        $payloadEncoder = $this->createMock(PayloadEncoderInterface::class);
        $payloadEncoder
            ->expects($this->once())
            ->method('encode')
            ->with($payload)
